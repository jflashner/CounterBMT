...
INPUT_INDEX="${INPUT_INDEX:-/path/to/sdc_path_control_index.jsonl}"
FORWARD_CKPT="${FORWARD_CKPT:-/path/to/forward_only.ckpt}"
OUTDIR="${OUTDIR:-$REPO_ROOT/logs/pr10_sdc_path_control}"
...
WANDB_GROUP="${WANDB_GROUP:-pr10_sdc_path_control}"
...
resolve_indexes() {
  ...
  if [[ -d "$input_index" ]]; then
    if [[ -f "$input_index/sdc_path_control_index_train.jsonl" ]]; then
      train_index="$input_index/sdc_path_control_index_train.jsonl"
      val_index="$input_index/sdc_path_control_index_val.jsonl"
    elif [[ -f "$input_index/sdc_path_control_index.jsonl" ]]; then
      train_index="$input_index/sdc_path_control_index.jsonl"
      val_index="$input_index/sdc_path_control_index.jsonl"
    fi
...
python src/Adv-BMT/bmt/train_motion.py \
  --config-name motion_forward_sdc_path_control_strict_local.yaml \
  ...
  DATA.COUNTERFACTUAL_MODE="sdc_path" \
