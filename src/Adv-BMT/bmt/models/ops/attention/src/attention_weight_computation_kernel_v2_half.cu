...
__global__ void attention_weight_computation_forward_v2_half(
    ...
    const at::BFloat16 *query_features, const at::BFloat16* key_features,
    at::BFloat16 *output) {
    ...
    __shared__ at::BFloat16 shared_query_features[d];
    ...
    at::BFloat16 attn_weight = __int2bfloat16_rn(0);
...
void attention_weight_computation_launcher_v2_half(
    int b, int total_query_num, int local_size,
    int total_key_num, int nhead, int hdim,
    const int *query_batch_cnt, const int *key_batch_cnt, const int* index_pair_batch,
    const int *index_pair,
    ...
    switch(hdim){  // switch hdim for utilizing different shared vectors.
        case 16:
            attention_weight_computation_forward_v2_half<16><<<blocks, threads>>>(
            ...
            break;
        case 24:
            attention_weight_computation_forward_v2_half<24><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                output);
            break;
        case 32:
            attention_weight_computation_forward_v2_half<32><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                output);
            break;
        case 48:
            attention_weight_computation_forward_v2_half<48><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                output);
            break;
        case 64:
            attention_weight_computation_forward_v2_half<64><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                output);
            break;
        case 128:
            attention_weight_computation_forward_v2_half<128><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                output);
            break;
        default:
            attention_weight_computation_forward_v2_half<150><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                output);
...
template <unsigned int d>
__global__ void attention_weight_computation_backward_v2_half(
    int b, int total_query_num, int local_size,
    int total_key_num, int nhead, int hdim,
    const int *query_batch_cnt, const int *key_batch_cnt, const int* index_pair_batch,
    const int *index_pair,
    const at::BFloat16 *query_features, const at::BFloat16* key_features,
    at::BFloat16 *grad_out, at::BFloat16 * grad_query_features, at::BFloat16 * grad_key_features) {
    // dim3 blocks(total_query_num, nhead); dim3 threads(local_size);
    // params query_batch_cnt: [b]
    // params key_batch_cnt: [b]
    // params index_pair_batch: [total_query_num]
    ...
    // build shared query features.
    __shared__ at::BFloat16 shared_query_features[d];
    __shared__ at::BFloat16 shared_grad_query_features[d];
    for (int i = local_key_idx; i < hdim; i += blockDim.x){
        shared_query_features[i] = query_features[
            query_idx * nhead * hdim + head_idx * hdim + i];
        shared_grad_query_features[i] = __int2bfloat16_rn(0);
    }
    __syncthreads();
    ...
    if (index_pair[index] != -1){
        ...
        at::BFloat16 gradient = grad_out[index * nhead + head_idx];
        for (int i = 0; i < hdim; i++){
//            atomicAdd(
...
void attention_weight_computation_grad_launcher_v2_half(
    int b, int total_query_num, int local_size,
    int total_key_num, int nhead, int hdim,
    const int *query_batch_cnt, const int *key_batch_cnt, const int* index_pair_batch,
    const int *index_pair,
    ...
    switch(hdim){
        case 16:
            attention_weight_computation_backward_v2_half<16><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
            ...
            break;
        case 24:
            attention_weight_computation_backward_v2_half<24><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                grad_out, grad_query_features, grad_key_features);
            break;
        case 32:
            attention_weight_computation_backward_v2_half<32><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                grad_out, grad_query_features, grad_key_features);
            break;
        case 48:
            attention_weight_computation_backward_v2_half<48><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                grad_out, grad_query_features, grad_key_features);
            break;
        case 64:
            attention_weight_computation_backward_v2_half<64><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                grad_out, grad_query_features, grad_key_features);
            break;
        case 128:
            attention_weight_computation_backward_v2_half<128><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                grad_out, grad_query_features, grad_key_features);
            break;
        default:
            attention_weight_computation_backward_v2_half<150><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, query_features, key_features,
                grad_out, grad_query_features, grad_key_features);
