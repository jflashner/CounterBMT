...
__global__ void attention_value_computation_forward_v2_half(
    ...
    const at::BFloat16 *attn_weight, const at::BFloat16* value_features,
    at::BFloat16 *output) {
    ...
    __shared__ at::BFloat16 shared_attn_weight[d];  // d == local_size
    ...
    at::BFloat16 attn_result = __int2bfloat16_rn(0);
...
void attention_value_computation_launcher_v2_half(
    int b, int total_query_num, int local_size,
    int total_key_num, int nhead, int hdim,
    const int *query_batch_cnt, const int *key_batch_cnt, const int* index_pair_batch,
    const int *index_pair,
    ...
    switch (local_size){
        case 16:
            attention_value_computation_forward_v2_half<16><<<blocks, threads>>>(
            ...
            break;
        case 32:
            attention_value_computation_forward_v2_half<32><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                output);
            break;
        case 64:
            attention_value_computation_forward_v2_half<64><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                output);
            break;
        case 128:
            attention_value_computation_forward_v2_half<128><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                output);
            break;
        case 320:
            attention_value_computation_forward_v2_half<320><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                output);
            break;
        case 384:
            attention_value_computation_forward_v2_half<384><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                output);
            break;
        default:
            attention_value_computation_forward_v2_half<512><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                output);
...
__global__ void attention_value_computation_backward_v2_half(
    int b, int total_query_num, int local_size,
    int total_key_num, int nhead, int hdim,
    const int *query_batch_cnt, const int *key_batch_cnt, const int* index_pair_batch,
    const int *index_pair,
    const at::BFloat16 *attn_weight, const at::BFloat16* value_features,
    at::BFloat16 *grad_out, at::BFloat16 * grad_attn_weight, at::BFloat16 * grad_value_features) {
    // dim3 blocks(total_query_num, nhead); dim3 threads(hdim);
    // params query_batch_cnt: [b]
    // params key_batch_cnt: [b]
    // params index_pair_batch: [total_query_num]
    ...
    // get shared variables.
    __shared__ at::BFloat16 shared_attn_weight[d], shared_grad_attn_weight[d];  // d == local_size
    __shared__ int shared_value_indices[d];
    for (int i = hdim_idx; i < local_size; i += blockDim.x){
        shared_attn_weight[i] = attn_weight[
            query_idx * local_size * nhead + i * nhead + head_idx];
        shared_grad_attn_weight[i] = __int2bfloat16_rn(0);
    ...
    at::BFloat16 gradient = grad_out[query_idx * nhead * hdim + head_idx * hdim + hdim_idx];
    for (int i = 0; i < local_size; i++){
        if (shared_value_indices[i] == -1) continue;
...
void attention_value_computation_grad_launcher_v2_half(
    int b, int total_query_num, int local_size,
    int total_key_num, int nhead, int hdim,
    const int *query_batch_cnt, const int *key_batch_cnt, const int* index_pair_batch,
    const int *index_pair,
    ...
    switch(local_size){
        case 16:
            attention_value_computation_backward_v2_half<16><<<blocks, threads>>>(
                 b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
            ...
            break;
        case 32:
            attention_value_computation_backward_v2_half<32><<<blocks, threads>>>(
                 b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                grad_out, grad_attn_weight, grad_value_features);
            break;
        case 64:
            attention_value_computation_backward_v2_half<64><<<blocks, threads>>>(
                 b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                grad_out, grad_attn_weight, grad_value_features);
            break;
        case 128:
            attention_value_computation_backward_v2_half<128><<<blocks, threads>>>(
                 b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                grad_out, grad_attn_weight, grad_value_features);
            break;
        case 320:
            attention_value_computation_backward_v2_half<320><<<blocks, threads>>>(
                 b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                grad_out, grad_attn_weight, grad_value_features);
            break;
        case 384:
            attention_value_computation_backward_v2_half<384><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                grad_out, grad_attn_weight, grad_value_features);
            break;
        default:
           attention_value_computation_backward_v2_half<512><<<blocks, threads>>>(
                b, total_query_num, local_size, total_key_num, nhead, hdim,
                query_batch_cnt, key_batch_cnt, index_pair_batch,
                index_pair, attn_weight, value_features,
                grad_out, grad_attn_weight, grad_value_features);
