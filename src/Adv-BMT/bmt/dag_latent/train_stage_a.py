"""Stage-A training entrypoint for additive DAG-latent legacy Adv-BMT.

This intentionally reuses the legacy training stack almost verbatim:
- same Hydra config flow
- same datamodule
- same callbacks/loggers
- same optimizer/scheduler through the Lightning module

The only functional difference is the Lightning module class:
`MotionLMDAGLatentLightning` swaps in the additive DAG-latent model wrapper.

For Stage A the config sets `DAG_LATENT.DAG_DROPOUT_PROB=1.0`, so even if DAG
inputs are present they have zero effect and the run reduces to the plain
autoregressive legacy recipe.
"""
...
# When this repo is launched from the workspace root, the local `metadrive/`
# and `scenarionet/` checkouts are discovered as namespace package shells.
# Prepending the package roots keeps legacy imports working without touching
# shared dataset code.
...
@hydra.main(
    version_base=None,
    config_path=str(REPO_ROOT / "cfgs"),
    config_name="0202_midgpt_dag_stage_a.yaml",
)
def main(config):
    ...
    if pretrained_path:
        ...
        # Stage-A DAG-latent adds a few new parameters that legacy base
        # checkpoints do not contain. `strict=False` lets us warm-start from a
        # plain legacy MidGPT checkpoint while initializing DAG params freshly.
        ...
        model = MotionLMDAGLatentLightning(config=config)
    model.exp_name = name
